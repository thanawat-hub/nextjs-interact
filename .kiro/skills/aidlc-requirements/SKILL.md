...
On approval: update manifest (set `artifacts.requirements.status` to `"approved"`, add `"requirements"` to `state.sharedPhases`). Store team size from D1 in `context-summary.teamSize` (e.g., `"solo"`, `"small"`, `"medium"`, `"large"`). Append audit entry. If platform is Claude Code, update `.claude/CLAUDE.md` (set Phase to "requirements"). Then auto-continue to routing-decision (below).
