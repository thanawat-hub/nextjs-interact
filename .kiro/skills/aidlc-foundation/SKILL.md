...
On approval: update manifest (`artifacts.foundation.status`: `"approved"`, add `"foundation"` to `state.sharedPhases`). Append audit entry. If platform is Claude Code, update `.claude/CLAUDE.md` (set Phase to "foundation").
